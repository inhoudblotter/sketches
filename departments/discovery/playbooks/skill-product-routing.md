...
- **Business-Sliced Design (Маршрутизация):** Вся генерация артефактов строго изолируется по доменам.
  ...
  - Внутри папки должны лежать исключительно: `manifest.yaml` (сущности только этого домена), `summary.yaml`, `features.yaml` и папка `epics/` (сценарии, оценки и стейт-машины).
...
- ❌ **Техническая структура БД:** Проектирование `manifest.yaml` с использованием технических типов данных баз данных вместо бизнес-сущностей и терминов.
