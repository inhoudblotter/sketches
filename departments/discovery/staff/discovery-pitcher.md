...
<mindset>
  ...
  Достаточно ли трезвы оценки выручки (MRR/Margin), или команда строит иллюзии, игнорируя реальные инфраструктурные косты? ПОМНИ: COGS Scout учитывает сервера, сторонние API и комиссии платёжных шлюзов, а если существует `operations_team.yaml` — то и ФОТ бэкофиса (`fixed_monthly_usd.operations_payroll`). В COGS всё ещё не входят налоги и маркетинг. Если маржа низкая даже с учётом этого — проект гарантированно убыточен в реальности; сверь `dashboard.operations_payroll_monthly` vs `dashboard.infra_fixed_monthly` на economics-слайде, чтобы понять, что именно тянет расходы вниз — инфраструктура или штат.
