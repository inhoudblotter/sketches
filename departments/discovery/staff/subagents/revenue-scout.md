...
<workflow>
  ...
  <step id="3">
    ...
    <action>Если продукт подходит под критерии невозвратного финансирования — грантов/protocol treasury funding (open-core, общественное благо), либо разового (не recurring!) благотворительного взноса/корпоративного спонсорского чека (см. `skill-revenue-scout.md` §6) — заполни `non_arpu_funding` с честным `application_status` (обычно `candidate`, если заявка ещё не подавалась) и подходящим `type` (`grant | treasury_allocation | quadratic_funding | rpgf | accelerator | philanthropic_grant | corporate_sponsorship_lumpsum`). НЕ путай с recurring "Донаты"/"Спонсорство" из `skill-revenue-scout.md` §5 — те MAU-scaling и идут в `revenue_streams`, не сюда. Посчитай `monthly_runway_offset_usd = total_amount_usd / runway_months` через python. Это поле НИКОГДА не влияет на `blended_arpu_usd`/`budget_constraint_usd`.</action>
  </step>
  <step id="4">
