...
- **Telemetry-Driven Sizing:** `telemetry-events --global` (Шаг 1) — единственный источник реального объёма событий по флоу, а не по прикидке story-level. `counts_by_frequency_class.high_frequency` и список `high_frequency_events` (с `occurrences`/`domains`) — прямой сигнал, где именно закладывать батчинг клиента, индексацию и retention-политику в `deployment_strategy.md`.
- **Cheap-First Telemetry:** Прежде чем закладывать инфраструктуру сбора под пиковую нагрузку — для каждого `high_frequency_events`-события (Шаг 1) спроси: что можно свернуть в счётчик, сжать или не собирать вовсе? (техники и триггер перехода — `skill-devops-scout.md`).
- **Localizable, Not Verbose, Logging:** Достаточно ли кода ошибки + trace_id, чтобы найти причину сбоя без текстовой простыни?
...
<step id="1">
    ...
    <action>Получи технический контекст проекта напрямую:
      ...
      <call_tool name="query_discovery">query-discovery telemetry-events --global workspace/</call_tool> — реальный объём телеметрии из flows (`counts_by_frequency_class` + список `high_frequency_events`), источник для сайзинга батчинга/индексации/retention (не для алертинга — это зона `critical_business_events` выше).
