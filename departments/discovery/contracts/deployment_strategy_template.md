...
Рекомендуемый стек для Observability. Принцип: дешёвые техники (батчинг, коды вместо текста, агрегирование) — по умолчанию, тяжёлая инфраструктура сбора — только под явно найденный `high_frequency_events`-триггер (Шаг 1). Конкретный формат логов и схема кодов ошибок — предмет архитектурной фазы, не discovery.
